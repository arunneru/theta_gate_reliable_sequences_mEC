...
#include "currents/I_transientPulse.hpp"
...
class N_InterNeuron_Wang96 : public Neuron {
 public:
  void ode_set(state_type &variables, state_type &dxdt, const double t, unsigned index) {
    ...
    I_transientPulse::current(variables,dxdt,t,index);
    ...
    double I_transientPulse = engine::neuron_value(index, "I_transientPulse");
    ...
    engine::neuron_value(index,"I_transientPulse",I_transientPulse);
    
    dxdt[v_index] = I_Ext + I_DC + I_transientPulse - I_Na - I_K - I_Leak - I_syn - I_Noise - I_OscillatoryDrive;
  } // function ode_set
