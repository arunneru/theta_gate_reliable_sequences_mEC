...
#ifndef INCLUDED_I_TRANS_HPP
#define INCLUDED_I_TRANS_HPP

#include "insilico/core/engine.hpp"

namespace insilico {

class I_transientPulse {
 public:
  static void current(state_type &variables, state_type &dxdt, const double t, int index) {
    ...
    double shift = engine::neuron_value(index, "shiftInMsec");
    ...
    if (index == 3){
      ...
      int cyc_num = int(t/PulseDuration);
      if (cyc_num == 24){
        T = fmod(t,PulseDuration);
        neur_ind = index - numIs ;
        PulseStart =  PulseDuration - (neur_ind + 1) * (PulseDuration / numIs) + shift;
        PulseEnd = PulseStart + pulsewidth + shift;//( neur_ind + 1 ) * (PulseDuration / numEs);
        P = PulseMin;
        if (T < PulseStart){
          P = PulseMin;
        }     
    ...
    // Current
    engine::neuron_value(index, "I_transientPulse", P);
