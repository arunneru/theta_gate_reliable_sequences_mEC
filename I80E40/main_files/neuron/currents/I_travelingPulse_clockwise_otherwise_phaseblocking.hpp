...
class I_travelingPulse {
 public:
  static void current(state_type &variables, state_type &dxdt, const double t, int index) {
    ...
    double phase = engine::neuron_value(index, "phaseOne");
    double phasedifference = engine::neuron_value(index, "phasedifference");
    ...
    if (index < 80){
      neur_ind = index - numEs ;
      PulseStart =  PulseDuration - (neur_ind + 1) * (PulseDuration / numEs) +  (phase/(2*M_PI)) * (PulseDuration / numEs);
      ...
      neur_ind = index - 2*numEs ;
      fin_phase = fmod((phase + phasedifference),2*M_PI);
      PulseStart = neur_ind * (PulseDuration / numEs) + (fin_phase/(2*M_PI)) * (PulseDuration / numEs);
